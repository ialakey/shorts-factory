...
### `debug`
- `true`: использует `test_data/` и `output_test_data/`, автозагрузка отключается.
...
### `kodik_download`
Параметры автозагрузки тайтлов через
[anime-dl-core](https://github.com/ialakey/anime-dl-core). Список записей, каждая — либо
последовательность, либо словарь:

```yaml
kodik_download:
  - ["Название", "1-3", "AniLibria"]              # тайтл, серии, озвучка
  - ["Название", "1-3", "AniLibria", 720, "kodik"] # + максимальное качество и плеер
  - {title: Название, episodes: "1-3", voice: AniLibria, quality: 720}
```

- `episodes` — `"1"`, `"1-3"`, `"1,4,7"` или их сочетание;
- `voice` — необязательна, подбирается по похожести названия озвучки;
- `quality` (`max_quality`) — верхняя граница по высоте картинки; если под неё не подошёл
  ни один поток, ограничение снимается;
- `player` — жёстко зафиксировать плеер (`kodik`, `cvh`, `aniboom`, `sibnet`, ...).

Токен не нужен. Для серии перебираются все плееры, которые отдал сайт: берётся первый,
вернувший рабочий поток. Прямой `mp4` качается через `requests`, `hls`/`dash` — через `ffmpeg`.
...
- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_CHAT_ID`
- `ANIMEGO_MIRROR` — зеркало каталога, если `animego.org` заблокирован (например `animego.me`)
- `ANIME_DL_PROXY` — прокси для этапа скачивания (`http://host:port` или `socks5://host:port`)
